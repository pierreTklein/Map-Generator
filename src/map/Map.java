...
public class Map implements Serializable{
	...
	/**Setting if you just want one biome:**/
	private boolean oneBiome = false;
	private Biome chosenBiome = biomeMap.getBiome(0.2,0.3); //grasslands by default.
	 ...
	 * @return the oneBiome
	 */
	public boolean isOneBiome() {
		return oneBiome;
	}
	/**
	 * @param oneBiome the oneBiome to set
	 */
	public void setOneBiome(boolean oneBiome) {
		this.oneBiome = oneBiome;
	}
	/**
	 * @return the chosenBiome
	 */
	public Biome getChosenBiome() {
		return chosenBiome;
	}
	/**
	 * @param chosenBiome the chosenBiome to set
	 */
	public void setChosenBiome(Biome chosenBiome) {
	...
	public void generateMapWithBiome(float xStart, float yStart, float step, float scale, Canvas mapCanvas){
		if(oneBiome != true){
			...
			generateMapWithOneBiome(mapCanvas, this.chosenBiome);
		}
	}
	/**Debug setting: if you just want one biome:**/
	public void generateMapWithOneBiome(Canvas mapCanvas, Biome biome){
		GraphicsContext gc = mapCanvas.getGraphicsContext2D();
		PixelWriter writer = gc.getPixelWriter();
		for(int i = 0; i < mapCanvas.getHeight(); i++){
			for(int j = 0; j < mapCanvas.getWidth(); j++){
